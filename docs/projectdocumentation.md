...
- A fictional Product B is defined in code only for comparison purposes. [file:8]
...
- **Orchestrator Agent**: Defines the end-to-end flow: parse → questions → content blocks → templates → JSON files. [file:8]
...
5. Orchestrator serializes the page models to JSON in `outputs/`. [file:8]
...
- Templates define which fields and logic blocks populate each page type, so new page types can be added by composing existing blocks. [file:8]
